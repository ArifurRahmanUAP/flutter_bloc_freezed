...
class MovieBookmarkBloc extends Bloc<MovieBookmarkEvent, MovieBookmarkState> {
  ...
  MovieBookmarkBloc(
      this.addMoviesToBookmarkUseCase, this.getBookmarksMovieUseCase, this.deleteBookmarkUseCase)
      ...
      (event, emit) async {
        await event.map(
          addToBookmarkEvent: (value) async {
            ...
            final response = await addMoviesToBookmarkUseCase(value.movieDetails);
          ...
          getBookmarkEvent: (value) async {
            emit(state.copyWith(isLoading: true));
            final response =
                await getBookmarksMovieUseCase(NoParams());
            await response?.fold((failure) async {
              emit(state.copyWith(
                  isLoading: false,
                  isFailure: true,
